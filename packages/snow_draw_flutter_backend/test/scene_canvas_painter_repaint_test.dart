...
void main() {
  test('repaints when document scene version changes after style updates', () {
    final registry = DefaultElementRegistry();
    registerBuiltInElements(registry);

    const baseElement = ElementState(
      id: 'rect-1',
      rect: DrawRect(minX: 0, minY: 0, maxX: 100, maxY: 80),
      rotation: 0,
      opacity: 1,
      zIndex: 0,
      data: RectangleData(),
    ...
    final initialState = DrawState(
      domain: DomainState(document: DocumentState(elements: [baseElement])),
    );
