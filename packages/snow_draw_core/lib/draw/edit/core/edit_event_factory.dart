...
/// Builds edit events from edit results.
@immutable
class EditEventFactory {
  const EditEventFactory();

  EditSessionEvent? forStartResult(
    EditResult result,
  ...
  EditSessionEvent? forUpdateResult(EditResult result) {
    if (result.isFailure) {
  ...
  EditSessionEvent? forFinishResult(EditResult result) {
    if (result.isFailure) {
  ...
  EditSessionEvent forCancelResult(EditResult result) {
    if (result.isSuccess) {
      return EditCancelled(operationId: result.operationId);
    }
