...
const _allowedEngineImport = 'package:snow_draw_core/snow_draw_engine.dart';
...
void main() {
  ...
  for (final testPath in _compatibilityTests) {
    ...
    for (var index = 0; index < lines.length; index++) {
      ...
      if (importTarget != _allowedEngineImport) {
        violations.add(
          '$testPath:${index + 1} imports "$importTarget" '
          '(expected "$_allowedEngineImport")',
        );
      }
    }
  }
  ...
  if (violations.isEmpty) {
    stdout.writeln(
      'Backend compatibility tests use only the engine entrypoint import.',
    );
    return;
  }

  stderr.writeln('Engine import boundary violations detected:');
