...
void main() {
  testWidgets('hidden dependent descriptors are not evaluated', (tester) async {
    final fillColorCounters = _Counters();
    final fillStyleCounters = _Counters();
    PropertyRegistry.instance
      ..clear()
      ..register(_CountingFillColorDescriptor(fillColorCounters))
      ..register(_CountingFillStyleDescriptor(fillStyleCounters));
    ...
    expect(fillColorCounters.extractCalls, 1);
    expect(fillColorCounters.defaultCalls, 1);
    expect(fillStyleCounters.extractCalls, 0);
    expect(fillStyleCounters.defaultCalls, 0);
  });
}
...
class _CountingFillStyleDescriptor extends PropertyDescriptor<FillStyle> {
  _CountingFillStyleDescriptor(this.counters)
    : super(
  ...
  MixedValue<FillStyle> extractValue(StylePropertyContext context) {
    counters.extractCalls += 1;
    return const MixedValue(value: FillStyle.solid, isMixed: false);
  }
