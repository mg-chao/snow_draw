...
void main() {
  ...
  if (!backendTestRoot.existsSync()) {
    stderr.writeln('Missing backend test root: $_backendTestRootPath');
    exitCode = 1;
    return;
  }

  final backendTestFiles = backendTestRoot
      .listSync(recursive: true, followLinks: false)
      .whereType<File>()
      .where((file) => file.path.endsWith('.dart'))
      .toList(growable: false);

  for (final file in backendTestFiles) {
    ...
    for (var index = 0; index < lines.length; index++) {
      ...
      violations.add(
        '$normalizedPath:${index + 1}: backend tests must reference core via '
        'package:snow_draw_core/snow_draw_core.dart',
      );
    }
  }

  if (backendTestFiles.isEmpty) {
    violations.add('$_backendTestRootPath: no Dart files found');
  }

  if (violations.isNotEmpty) {
    stderr.writeln('Backend test core import boundary check failed:');
  ...
  stdout.writeln(
    'Backend test core import boundary check passed. Backend tests import '
    'core APIs through the core entrypoint only.',
  );
}
