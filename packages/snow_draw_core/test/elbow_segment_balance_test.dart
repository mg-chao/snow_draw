...
void main() {
  test(
    'right-down-right path balances first and last horizontal segments',
    () {
      // Rectangle A on the left, Rectangle B on the right,
      // with B offset downward so the path is [right, down, right].
      const rectA = DrawRect(
        minX: 100,
        minY: 100,
        maxX: 200,
        maxY: 200,
      );
      const rectB = DrawRect(
        minX: 400,
        minY: 150,
        maxX: 500,
        maxY: 250,
  ...
  test(
    'down-right-down path balances first and last vertical segments',
    () {
  ...
  test(
    'right-up-right path balances when B is above A',
    () {
      const rectA = DrawRect(
        minX: 100,
        minY: 200,
...
void _expectBalancedThreeSegmentPath({
  ...
  final startBinding = ArrowBinding(
    elementId: 'a',
    anchor: startAnchor,
  );
  final endBinding = ArrowBinding(
    elementId: 'b',
    anchor: endAnchor,
  );
